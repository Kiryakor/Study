...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        LoadDoctors.loadData()
        LoadPatient.loadData()
        LoadStatistic.loadData()
        LoadDoctors.loadDataHistory()
    ...
    @objc func sceneWillResignActiveNotification(_ notification: NSNotification) {
        SaveDoctors.saveData()
        SavePatient.saveData()
        SaveStatistic.saveData()
        SaveDoctors.saveDataHistory()
    }

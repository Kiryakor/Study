...
class Menu {
    ...
    func menu() {
        ...
        while check {
            ...
            print("11 - поиск пассажира по N авиарейса. Результаты поиска – все сведения о найденном авиарейсе. Результаты поиска – все сведения о найденном авиарейсе, а также ФИО и номера паспортов пассажиров, которые купили билет на этот авиарейс")
        ...
        if Flight.checkNumber(number: number!){
            ...
            for i in data{
                if i.number == number!{
                    check = false
                    i.printData()
                    list.passportInfo(numberAirFlight: number!, table: table)
                }
            }
            if check{ print("Данного авиарейса нету") }
        }else{ print("Вы ввели неверно данные") }
    }
